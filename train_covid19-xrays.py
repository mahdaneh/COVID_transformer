...
CONFIG_FILE = "configs/Res_VIT.json"
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(kfolds.split(dataset, dataset.labels)):
        ...
        logger.info(
            "%s model on device: %s", net_name, next(network.parameters()).is_cuda
        )
        ...
        with logging_redirect_tqdm():
            ...
            torch.save(
                network.state_dict(),
                CONFIG_FILE.replace(".log", "{_fold_%s}.pth" % fold),
            )
