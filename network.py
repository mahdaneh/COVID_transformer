...
class InputEmbbeding(nn.Module):
    def __init__(self, image_size, patch_size,  emb_dim):
        ...
        self.pos_embed = nn.Parameter(torch.randn(1, 1+self.num_patches, self.emb_dim))
    def forward(self, x):
        n_batches = x.size(0)
        x = x.view( n_batches, self.num_patches , self.input_dim)
        #linear projection
        x  = self.linear_project(x)
        cls_token = self.cls_token.expand(n_batches, -1, -1)  # Shape: (b, 1, self.emb_dim)
...
class TransformerEncoderBlock(nn.Module):
    ...
    def forward(self,x):
        ...
        x_msa = torch.cat([self.heads[i](x_norm) for i in range(len(self.heads))],dim=2)
...
class VIT(nn.Module):
    def __init__(self, config):
        ...
        if embed_dim % num_heads != 0: embed_dim +=  (embed_dim % num_heads)

        self.input_embed = InputEmbbeding(image_size, patch_size=patch_size, emb_dim=embed_dim)
        self.encoders = nn.ModuleList([TransformerEncoderBlock(embed_dim, num_heads) for _ in range(num_layers)])
        self.MLP_cls = nn.Sequential(nn.Linear(embed_dim, num_classes, bias=True), nn.Softmax(dim=1))
